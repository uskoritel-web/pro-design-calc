...
export function calcTotal(data, settings) {
  ...
  const каменьБелыйМетры = parseFloat(data.каменьБелыйМетры) || 0;
  const каменьБелый = каменьБелыйМетры * (parseFloat(settings.столешницы?.каменьБелый) || 0);

  const мойкаКамень = (parseFloat(data.мойкаКамень) || 0) * (parseFloat(settings.столешницы?.мойкаКамень) || 0);
  const подклейкаМойки = (parseFloat(data.подклейкаМойки) || 0) * (parseFloat(settings.столешницы?.подклейкаМойки) || 0);
  const подклейка = (parseFloat(data.подклейка) || 0) * (parseFloat(settings.столешницы?.подклейка) || 0);
